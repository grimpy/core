...
class JellyfinMediaPlayer(JellyfinEntity, MediaPlayerEntity):
    ...
    def media_image_url(self) -> str | None:
        ...
        artwork_id: str = self.now_playing["Id"]
        artwork_type = "Primary"
        parent_backdrop_id: str | None = self.now_playing.get("ParentBackdropItemId")
        ...
        elif "Primary" not in self.now_playing[ITEM_KEY_IMAGE_TAGS]:
            return None

        return str(
            self.coordinator.api_client.jellyfin.artwork(artwork_id, artwork_type, 100)
        )
